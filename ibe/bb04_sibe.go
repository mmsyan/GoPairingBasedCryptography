...
//   - 加密(Encrypt)
//   - 解密(Decrypt)
// 考虑到同期还有一篇《Secure Identity Based Encryption Without Random Oracles》
// https://link.springer.com/chapter/10.1007/978-3-540-28628-8_27
// 这篇我们命名为BB04sIBE (表示它是selective-id secure)
// 另外一篇命名为BB04IBE (表示它是full secure)
...
// BB04sIBEInstance 表示Boneh-Boyen身份基加密(IBE)方案的实例对象。
// 该实例包含了系统的主密钥对(x, y)，其中x和y都是Zp域上的随机元素。
// 主密钥用于生成用户的私钥，必须严格保密。
type BB04sIBEInstance struct {
	x fr.Element
	y fr.Element
}

// BB04sIBEPublicParams 表示Boneh-Boyen IBE方案的公共参数。
// 这些参数在系统初始化时生成，可以公开发布给所有用户。
// 包含基础生成元g1和g2，以及由主密钥派生的公开元素x=g1^x和y=g1^y。
type BB04sIBEPublicParams struct {
	g1 bn254.G1Affine
	g2 bn254.G2Affine
	x  bn254.G1Affine
	y  bn254.G1Affine
...
// BB04sIBEIdentity 表示Boneh-Boyen IBE方案中的用户身份。
// 身份被编码为Zp有限域上的一个元素。
// 在实际应用中，可以通过哈希函数将任意字符串(如邮箱地址)映射到Zp域元素。
type BB04sIBEIdentity struct {
	Id fr.Element
}

// BB04sIBESecretKey 表示Boneh-Boyen IBE方案中的用户私钥。
// 私钥包含两个部分：
//   - r: Zp域上的随机元素
//   - k: G2群上的元素，计算为k=g2^{1/(Id+x+ry)}
//
// 私钥由密钥生成中心(PKG)使用主密钥和用户身份生成，必须安全地传递给用户。
type BB04sIBESecretKey struct {
	r fr.Element
	k bn254.G2Affine
}

// BB04sIBEMessage 表示Boneh-Boyen IBE方案中的明文消息。
// 明文被编码为GT群(配对运算的目标群)上的一个元素。
// 在实际应用中，通常需要将原始消息映射到GT群元素。
type BB04sIBEMessage struct {
	Message bn254.GT
}

// BB04sIBECiphertext 表示Boneh-Boyen IBE方案中的密文。
// 密文由三个部分组成：
//   - a: G1群上的元素，编码了身份和随机性
//   - b: G1群上的元素，编码了随机掩码
//   - c: GT群上的元素，包含加密后的消息
type BB04sIBECiphertext struct {
	a bn254.G1Affine
	b bn254.G1Affine
	c bn254.GT
}

// NewBB04sIBEInstance 创建一个新的Boneh-Boyen IBE方案实例。
// 该函数随机生成主密钥对(x, y)，两者都是从Zp域中均匀随机采样的元素。
// 返回的实例对象包含主密钥，应该由可信的密钥生成中心(PKG)持有并妥善保管。
//
// 返回值:
//   - *BB04sIBEInstance: 包含主密钥的IBE实例
//   - error: 如果随机数生成失败，返回错误信息
func NewBB04sIBEInstance() (*BB04sIBEInstance, error) {
	var err error
	var x, y fr.Element
	_, err = x.SetRandom()
	_, err = y.SetRandom()
	...
	return &BB04sIBEInstance{x, y}, nil
}
...
//
// 返回值:
//   - *BB04sIBEPublicParams: 系统公共参数，包含g1, g2, g1^x, g1^y
//   - error: 如果初始化失败，返回错误信息
func (instance *BB04sIBEInstance) SetUp() (*BB04sIBEPublicParams, error) {
...
//
// 返回值:
//   - *BB04sIBESecretKey: 生成的私钥，包含随机参数r和密钥元素k
//   - error: 如果密钥生成失败，返回错误信息
func (instance *BB04sIBEInstance) KeyGenerate(identity *BB04sIBEIdentity, publicParams *BB04sIBEPublicParams) (*BB04sIBESecretKey, error) {
	var err error
	var r fr.Element
	_, err = r.SetRandom()
...
//
// 返回值:
//   - *BB04sIBECiphertext: 加密后的密文，包含a, b, c三个组件
//   - error: 如果加密失败，返回错误信息
func (instance *BB04sIBEInstance) Encrypt(message *BB04sIBEMessage, identity *BB04sIBEIdentity, publicParams *BB04sIBEPublicParams) (*BB04sIBECiphertext, error) {
	var err error
	var s fr.Element
	_, err = s.SetRandom()
	if err != nil {
	...
	return &BB04sIBECiphertext{*a, b, c}, nil
}
...
//
// 返回值:
//   - *BB04sIBEMessage: 解密后的明文消息
//   - error: 如果解密失败，返回错误信息
func (instance *BB04sIBEInstance) Decrypt(ciphertext *BB04sIBECiphertext, secretKey *BB04sIBESecretKey, publicParams *BB04sIBEPublicParams) (*BB04sIBEMessage, error) {
	// A*B^r
