...
func TestWaters05Ibe1(t *testing.T) {
	...
	identity, err := NewWatersIdentity(identityString)
	if err != nil {
...
func TestWaters05Ibe2(t *testing.T) {
	...
	bobIdentity, err := NewWatersIdentity(bobIdentityString)
	if err != nil {
...
func TestWaters05Ibe3(t *testing.T) {
	...
	// 创建三个用户身份 (使用不同的字符串)
	alice, err := NewWatersIdentity("alice-user-id-1001")
	bob, err := NewWatersIdentity("bob-user-id-2002")
	charlie, err := NewWatersIdentity("charlie-user-id-3003")
...
func TestWaters05Ibe4(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// 1. 创建身份
			identity, err := NewWatersIdentity(tc.identityStr)
