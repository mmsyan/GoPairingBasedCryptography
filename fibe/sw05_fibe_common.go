...
type SW05FIBEInstance struct {
	...
	msk_ti   map[int64]fr.Element // 主密钥组件 t_i，是 Zq 域上的随机元素数组。
...
type SW05FIBEPublicParams struct {
	...
	pk_Ti map[int64]*bn254.G2Affine // 公钥组件 T_i = g2^t_i，对应第 i 个属性。
...
type SW05FIBESecretKey struct {
	...
	di             map[int64]*bn254.G1Affine // 私钥组件 D_i，对应 S_user 中的每个属性 i。
...
func NewSW05FIBEInstance(universe int64, distance int) *SW05FIBEInstance {
	// 使用 &SW05FIBEInstance{} 语法创建一个结构体实例并返回其指针。
	return &SW05FIBEInstance{
		universe: universe,
		distance: distance,
		// Zp类型元素数组。为了与论文适配，我们选择让属性从1开始到U结束，
		msk_ti: make(map[int64]fr.Element),
	}
}
...
func (instance *SW05FIBEInstance) KeyGenerate(userAttributes *SW05FIBEAttributes, publicParams *SW05FIBEPublicParams) (*SW05FIBESecretKey, error) {
	...
	if !utils2.CheckAttributesArray(userAttributes.attributes, instance.universe) {
	...
	for _, i := range userAttributes.attributes {
		// 计算 q(i)。
		qi := utils.ComputePolynomialValue(polynomial, *new(fr.Element).SetInt64(i))
...
func (instance *SW05FIBEInstance) Encrypt(messageAttributes *SW05FIBEAttributes, message *SW05FIBEMessage, publicParams *SW05FIBEPublicParams) (*SW05FIBECiphertext, error) {
	// 检查属性集是否有效。
	if !utils2.CheckAttributesArray(messageAttributes.attributes, instance.universe) {
...
func (instance *SW05FIBEInstance) Decrypt(userSecretKey *SW05FIBESecretKey, ciphertext *SW05FIBECiphertext, publicParams *SW05FIBEPublicParams) (*SW05FIBEMessage, error) {
	// 检查属性集是否有效。
	if !utils2.CheckAttributesArray(userSecretKey.userAttributes, instance.universe) {
		return nil, fmt.Errorf("invalid user attributes")
	}
	if !utils2.CheckAttributesArray(ciphertext.messageAttributes, instance.universe) {
		return nil, fmt.Errorf("invalid cipher text")
	}
...
func NewFIBEAttributes(attributes []int64) (*SW05FIBEAttributes, error) {
