// Package ibbe07
// implements the Identity-Based Broadcast Encryption (IBBE07).
// 作者: mmsyan
// 日期: 2026-01-31
// Delerablée, C. (2007). Identity-Based Broadcast Encryption with Constant Size Ciphertexts and Private Keys.
...
func Decrypt(s []Identity, id *Identity, sk *UserSecretKey, hdr *BroadcastHeader, pk *PublicKey) (*MessageEncyptionKey, error) {
	...
	if len(coeffs) > 1 {
		// 使用 coeffs[1] 对应 γ⁰ = 1，所以用 h
		hPower.ScalarMultiplication(&pk.H, coeffs[1].BigInt(new(big.Int)))
		// coeffs[2] 对应 γ¹，用 h^γ = HGammaPowers[0]
		for i := 2; i < len(coeffs); i++ {
			var tmp bn254.G2Affine
			tmp.ScalarMultiplication(&pk.HGammaPowers[i-2], coeffs[i].BigInt(new(big.Int)))
	...
	// 5. 计算 e(C1, h^{p_{i,S}(γ)}) · e(sk_{ID_i}, C2)
	...
	// 6. 计算最终指数 ∏_{j≠i} 1/H(ID_j) = 1/coeffs[0]
	expInverse := new(fr.Element).Inverse(&coeffs[0])
